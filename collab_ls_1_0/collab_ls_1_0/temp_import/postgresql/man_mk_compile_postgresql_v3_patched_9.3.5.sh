...
sudo apt-get install flex bison build-essential libreadline6-dev zlib1g-dev libossp-uuid-dev

version=3_64  # change this if you want to build several versions of postgres in parallel
# see also "MODIFY THIS TOO" below

echo "current version is"  $version
mkdir -p ~/bin/postgresql_9.3.5
cd ~/bin/postgresql_9.3.5
wget ftp://ftp.postgresql.org/pub/source/v9.3.5/postgresql-9.3.5.tar.bz2
mkdir -p ~/bin/postgresql_9.3.5/patched_$version
tar -xvf postgresql-9.3.*.tar.bz2 -C ~/bin/postgresql_9.3.5/patched_$version
cd patched_$version/postgresql-9.3.* 

# use kate (KDE) or your preferred text editor:
geany src/include/access/htup_details.h 
...
# or by this:    (in TWO PLACES)   (USED 7 Sept 2014)
...
# file "[...]/bin/postgresql_9.3.5/patched_3_64/postgresql-9.3.5/src/test/regress/regression.diffs".
# NOTE: those differences where only in the query plan, no big deal, see http://www.postgresql.org/message-id/53F4D326.8050508@free.fr
...
sudo su postgres
version=3_64   ############################## MODIFY THIS TOO IF CHANGED ABOVE
